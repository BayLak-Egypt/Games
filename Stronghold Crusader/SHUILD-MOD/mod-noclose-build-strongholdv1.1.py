...
BASE1_OFFSET = 0x110FECC
OFFSETS1 = []
IS_POINTER1 = False
BASE2_OFFSET = 0x137F5C8
OFFSETS2 = []
IS_POINTER2 = False
SLEEP_INTERVAL = 0.07
DATA_TYPE = 'int'
...
def resolve_final_address(pm, module_name, base_offset, offsets, is_pointer):
    """
    إذا is_pointer True: يفترض أن (module_base + base_offset) يحتوي على عنوان نقرا منه ثم نطبق offsets كـ pointer path.
    إذا is_pointer False: final address = module_base + base_offset  (تجاهل offsets، أو ممكن تستخدمها كـ إضافات ثابتة إذا أردت)
    """
    ...
    if not is_pointer:
        if offsets:
            final_addr = base_addr + offsets[-1]
        else:
    ...
    final_addr = addr + offsets[-1] if offsets else addr
    return final_addr
...
def main():
    ...
    while running:
        try:
            ...
            while running:
                try:
                    addr_first = resolve_final_address(pm, PROCESS_NAME, BASE1_OFFSET, OFFSETS1, IS_POINTER1)
                    addr_second = resolve_final_address(pm, PROCESS_NAME, BASE2_OFFSET, OFFSETS2, IS_POINTER2)
